(function() {
	...
	function detailButtonHtml(device) {
		if (!deviceHasDetailView(device)) return '';
		return '<button type="button" class="quicklook-primary-action" data-open-detail-location="' +
			escapeHtml(device.locationCode) + '">See More Details</button>';
	}

	function buildSitePopupHtml(device) {
		var title = device.displayTitle || device.locationName || device.locationCode || 'Site';
		var instruments = safeList(device.devices);
		var rows = instruments.map(function(d) {
			var color = d.statusColor || '#6c757d';
			return [
				'<li class="map-popup-instrument">',
				'  <span class="map-instr-dot" style="background:' + color + ';"></span>',
				'  <span class="map-instr-name">' + escapeHtml(d.deviceCode || d.deviceCategoryCode || '') + '</span>',
				'  <span class="map-instr-status" style="color:' + color + ';">' + escapeHtml(d.statusDisplay || '') + '</span>',
				'</li>'
			].join('');
		}).join('');
		...
		return [
			...
			'  <div class="map-popup-count">' + instruments.length + ' instrument' + (instruments.length === 1 ? '' : 's') + '</div>',
			'  <ul class="map-popup-instrument-list">' + (rows || '<li class="map-popup-empty">No instruments</li>') + '</ul>',
			'  <div class="detail-action-row">' + detailButtonHtml(device) + '</div>',
			'</div>'
		].join('');
	}

	function buildPopupHtml(device) {
		if (device && device.entityType === 'site') {
			return buildSitePopupHtml(device);
		}
		...
		var statusText = formatStatus(status.statusDisplay || status.overallStatus);
		var color = device.statusColor || statusColor(status.overallStatus);
		var codeLine = (device.deviceCategoryCode || '') + (device.sensorCode ? ' · ' + device.sensorCode : '');

		return [
			'<div class="map-popup-card">',
			'  <div class="map-popup-title">' + escapeHtml(title) + '</div>',
			subtitle ? '  <div class="map-popup-subtitle">' + escapeHtml(subtitle) + '</div>' : '',
			'  <div><span class="status-pill" style="background:' + color + ';color:#fff;">' + escapeHtml(statusText) + '</span></div>',
			codeLine ? '  <div class="map-popup-count">' + escapeHtml(codeLine) + '</div>' : '',
			'  <div class="detail-action-row">' + detailButtonHtml(device) + '</div>',
			'</div>'
		].join('');
	}
	...
	function applyInstrumentFilter(category) {
		if (!state.map) return;
		Object.keys(state.markers).forEach(function(code) {
			var marker = state.markers[code];
			var device = state.locationData[code];
			if (!marker || !device) return;
			var show = category === '__all__' || (device.deviceCategoryCode || '') === category;
			if (show) {
	...
	function initInstrumentFilter() {
		var select = document.getElementById('instrument-type-filter');
		if (!select) return;
		var categories = {};
		Object.keys(state.locationData).forEach(function(code) {
			var cat = state.locationData[code].deviceCategoryCode;
			if (cat) categories[cat] = true;
		});
		var options = ['<option value="__all__">All instrument types</option>'];
		Object.keys(categories).sort().forEach(function(cat) {
			options.push('<option value="' + escapeHtml(cat) + '">' + escapeHtml(cat) + '</option>');
		});
		select.innerHTML = options.join('');
		if (!select.dataset.bound) {
			select.addEventListener('change', function() { applyInstrumentFilter(select.value); });
	...
	function initMap() {
		...
		Object.keys(state.locationData).forEach(function(code) {
			...
			var marker = window.L.marker([Number(device.latitude), Number(device.longitude)], {
				icon: createMarkerIcon(device.statusColor || (device.status && statusColor(device.status.overallStatus)), offset),
				title: code
			});
		...
		if (markerCount === 0) {
			mapEl.innerHTML = '<div class="hydrophone-map-empty">No instrument coordinates are available for the current dataset.</div>';
			return;
		}
